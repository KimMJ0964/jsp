...
<body>
    ...
    <!-- 
    	*JSP를 이루는 구성
    	1. 스크립팅원소 : jsp 페이지내에서 자바 코드를 직접 기술할 수 있게 하는 기술
    		ex) 스크립틀릿, 표현식...
    	...
    	3. 액션태그 : JSP페이지에서 어떤 동작을 하도록 지시하는 태그
    		...
    		>> 표준 액션 태그(Standard Action Tag) : jps페이지에서 바로 사용 가능(별도로 연동 필요 없음)
    			-> 모든 태그명 앞에 jsp: 접두어를 붙여서 사용 <>내부에 jsp:태그명
    		>> 커스텀 액션 태그(Custom Action Tag) : JSP 페이지에서 바로 사용 불가능(별도로 라이브러리 연동 필요)
     ...
     <p>
     		...
     		공통적으로 자주 사용되는 코드들을 집합하여 보다 쉽게 사용할 수 있도록 태그화하여 표준으로 
     </p>
     
     <h3>라이브러리 추가</h3>
     1) 톰캣사이트 접속 <br>
     2) 메뉴바 Taglibs 메뉴로 이동 <br>
     3) 4개의 .jar 전부 다운로드 <br>
     4) WEB-INF/lib/() 추가 <br>
